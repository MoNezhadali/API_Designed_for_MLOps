...
class FlaskAPITests(unittest.TestCase):
    ...
    def test_missing_birthdate(self):
        ...
        response = requests.post(f"{self.API_URL}/primaryCompute", json={"email": "user@example.com"})
    ...
    def test_birthdate_category_minor(self):
        ...
        response = requests.post(f"{self.API_URL}/primaryCompute", json={"birthdate": birthdate})
        self.assertEqual(response.status_code, 200)
        data = response.json()
    ...
    def test_invalid_birthdate_format(self):
        ...
        response = requests.post(f"{self.API_URL}/primaryCompute", json={"birthdate": "1990/01/01", "email": "user@example.com"})
        self.assertEqual(response.status_code, 422)

    def test_invalid_birthdate_format(self):
        # Test with invalid birthdate format
        response = requests.post(f"{self.API_URL}/primaryCompute", json={"birthdate": "1990-15-16", "email": "user@example.com"})
        self.assertEqual(response.status_code, 422)
    
    def test_correct_digit_count(self):
        # Test correct number of digits
        response = requests.post(f"{self.API_URL}/primaryCompute", json={"email": "user123456@example.com"})
        self.assertEqual(response.status_code, 200)
        data = response.json()
